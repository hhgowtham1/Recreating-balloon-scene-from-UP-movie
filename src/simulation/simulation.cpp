...
bool simulate1(house_structure & house, std::vector<particle_structure>& balloons, float dt_true, std::vector<wire_structure>& wires)
{
	bool retur = true;
    ...
    vec3 const f_tension = k * (wire_len - balloon.l) * wire_dir;
        // f += parameters.wind.magnitude * coeff * n * L0 * L0;
			...
			f_buoyant.z =  rho_air * gravity * balloon.vol/100.f;
			vec3 f_gravity = vec3(0.f,0.f,0.f);
			f_gravity.z = (balloon.m +rho_helium * balloon.vol/100.f)*gravity;

			vec3 f_total = f_buoyant-f_gravity;//+f_tension; 
			...
			// std::cout<<f_cumulative<<" "<<f<<std::endl;
			
			if(f_cumulative.z> std::fabs(f.z) && 1)
			{
				retur = false;
